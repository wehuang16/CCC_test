...
model Hybrid_2024_v5_test "has cold and hot pcm charging in schedule"
  ...
  parameter Modelica.Units.SI.PressureDifference dpFixed_nominal = 1000 "pressure drop of pipe and other resistances that are in series" annotation(Dialog(group="System Specifications"));
    ...
    parameter Modelica.Units.SI.Time chargeStart_HotTes = 1*3*3600 "Continue charging Hot TES at 3am" annotation(Dialog(group="State Schedule: Summer"));
    parameter Modelica.Units.SI.Time chargeStart_ColdTes = 1*5.25*3600 "Continue charging Cold TES at 5:15am" annotation(Dialog(group="State Schedule: Summer"));
  ...
  parameter Modelica.Units.SI.Time dischargeEnd_Tes = 1*20*3600 "Stop discharging at 8pm"  annotation(Dialog(group="State Schedule: Summer"));
   ...
   Modelica.Blocks.Sources.CombiTimeTable SchTes(
    table=[0,0; chargeStart_HotTes,1; chargeStart_ColdTes,2; chargeEnd_Tes,0;
        dischargeStart_Tes,3; dischargeEnd_Tes,0; 24*3600,0],
    smoothness=Modelica.Blocks.Types.Smoothness.ConstantSegments,
    extrapolation=Modelica.Blocks.Types.Extrapolation.Periodic)
    "TES charging and discharging schedule for cooling. 0 = off, 1 = charge hot, 2 = charge cold, 3 = discharge"
    annotation (Placement(transformation(extent={{-364,70},{-344,90}})));
  ...
  Buildings.Fluid.Movers.FlowControlled_m_flow fan(
    ...
    addPowerToMedium=false) annotation (Placement(transformation(
        ...
        rotation=180,
        origin={-152,-142})));
  CCC.Controls.hybrid_2024_controller_new hybrid_2024_controller_new
  ...
  Modelica.Blocks.Interfaces.RealInput schedule_input
    annotation (Placement(transformation(extent={{-420,-6},{-380,34}})));
  Modelica.Blocks.Interfaces.RealOutput outdoor_air_temperature
    annotation (Placement(transformation(extent={{180,26},{200,46}})));
  Modelica.Blocks.Interfaces.BooleanInput mpc_enable
    annotation (Placement(transformation(extent={{-422,-92},{-382,-52}})));
  Modelica.Blocks.Logical.Switch switch1
    annotation (Placement(transformation(extent={{-350,24},{-330,44}})));
  Modelica.Blocks.Interfaces.RealOutput soc_cold_pcm
    annotation (Placement(transformation(extent={{182,0},{202,20}})));
  Modelica.Blocks.Interfaces.RealOutput soc_hot_pcm
  ...
  Modelica.Blocks.Interfaces.RealOutput fan_coil_fan_electric_power
    annotation (Placement(transformation(extent={{180,-202},{200,-182}})));
  Modelica.Blocks.Sources.Constant T_zon(k=0)
    annotation (Placement(transformation(extent={{98,-200},{118,-180}})));
  Modelica.Blocks.Interfaces.RealOutput pump_1_electric_power
    annotation (Placement(transformation(extent={{180,-84},{200,-64}})));
  Modelica.Blocks.Interfaces.RealOutput pump_2_electric_power
    annotation (Placement(transformation(extent={{180,-102},{200,-82}})));
  Modelica.Blocks.Interfaces.RealOutput heat_pump_electric_power
    annotation (Placement(transformation(extent={{180,-126},{200,-106}})));
  Modelica.Blocks.Interfaces.RealOutput heat_pump_mode
    annotation (Placement(transformation(extent={{180,-152},{200,-132}})));
  Modelica.Blocks.Interfaces.RealOutput heat_pump_useful_thermal_power
    annotation (Placement(transformation(extent={{180,-172},{200,-152}})));
equation
  ...
  connect(fanCoilUnit_hysteresis_air_control.FcuModeBoolean, AWHP.Mode)
    annotation (Line(points={{-136,-104},{-144,-104},{-144,-46},{-106,-46},{
          -106,-24.4},{-113.72,-24.4}},
                                      color={255,0,255}));
  connect(hybrid_2024_controller_new.FCUmode,
    fanCoilUnit_hysteresis_air_control.FcuMode) annotation (Line(points={{-284,
          41.2},{-292,41.2},{-292,-144},{-168,-144},{-168,-162},{-131.2,-162},{
          -131.2,-108}}, color={255,127,0}));
  connect(hybrid_2024_controller_new.SOChotPCM, hot_PCM.SOC) annotation (Line(
        points={{-284,28.4},{-78,28.4},{-78,-105.273},{127,-105.273}}, color={0,
          0,127}));
  connect(cold_PCM.SOC, hybrid_2024_controller_new.SOCcoldPCM) annotation (Line(
        points={{129,-21.2727},{128,-21.2727},{128,-8},{-70,-8},{-70,8},{-176,8},
          {-176,20},{-252,20},{-252,12},{-284,12},{-284,24.1333}}, color={0,0,
          127}));
  connect(hybrid_2024_controller_new.valA1, valA1.y) annotation (Line(points={{-257,
          51.8667},{-16,51.8667},{-16,-57},{68,-57}},      color={0,0,127}));
  connect(hybrid_2024_controller_new.valA2, valA2.y) annotation (Line(points={{
          -257,47.6},{-12,47.6},{-12,-78},{64,-78},{64,-87},{68.6,-87}}, color=
          {0,0,127}));
  connect(hybrid_2024_controller_new.valA4, valA4.y) annotation (Line(points={{-257,
          43.3333},{-257,-176},{-38,-176},{-38,-153.6}},      color={0,0,127}));
  connect(hybrid_2024_controller_new.valA5, valA5.y) annotation (Line(points={{-257,
          39.0667},{-257,-158},{-260,-158},{-260,-178},{45,-178},{45,-156}},
        color={0,0,127}));
  connect(hybrid_2024_controller_new.CP1, booleanToReal1.u) annotation (Line(
        points={{-257,34.8},{-257,-14},{-232,-14}}, color={255,0,255}));
  connect(hybrid_2024_controller_new.HP, booleanToReal3.u) annotation (Line(
        points={{-257,30.5333},{-257,-62},{-226,-62}}, color={255,0,255}));
  ...
  connect(AWHP.TSupSet, hybrid_2024_controller_new.HPset) annotation (Line(
        points={{-156.09,-32.07},{-166,-32.07},{-166,26.2667},{-257,26.2667}},
        color={0,0,127}));
  ...
  connect(fanCoilUnit_hysteresis_air_control.m_flow_air, fan.m_flow_in)
    annotation (Line(points={{-112.8,-104.2},{-112.8,-154},{-152,-154}}, color=
          {0,0,127}));
  connect(hybrid_2024_controller_new.HP, AWHP.OnOff) annotation (Line(points={{-257,
          30.5333},{-156.28,30.5333},{-156.28,-24.4}},      color={255,0,255}));
  connect(switch1.u2, mpc_enable) annotation (Line(points={{-352,34},{-370,34},
          {-370,-72},{-402,-72}}, color={255,0,255}));
  connect(SchTes.y[1], switch1.u3) annotation (Line(points={{-343,80},{-336,80},
          {-336,16},{-352,16},{-352,26}}, color={0,0,127}));
  connect(switch1.y, hybrid_2024_controller_new.scheduleInput) annotation (Line(
        points={{-329,34},{-294,34},{-294,51.8667},{-284.2,51.8667}}, color={0,
          0,127}));
  connect(switch1.u1, schedule_input)
    annotation (Line(points={{-352,42},{-400,42},{-400,14}}, color={0,0,127}));
  connect(RoomModel.TOut, outdoor_air_temperature) annotation (Line(points={{
          -199.8,-87},{-199.8,36},{190,36}}, color={0,0,127}));
  connect(RoomModel.ZoneTAir2, room_temperature) annotation (Line(points={{
          -184.4,-87},{-184.4,-48},{190,-48}}, color={0,0,127}));
  connect(cold_PCM.SOC, soc_cold_pcm) annotation (Line(points={{129,-21.2727},{
          154,-21.2727},{154,10},{192,10}}, color={0,0,127}));
  connect(hot_PCM.SOC, soc_hot_pcm) annotation (Line(points={{127,-105.273},{
          110,-105.273},{110,-20},{192,-20}}, color={0,0,127}));
  connect(T_zon.y, fan_coil_fan_electric_power) annotation (Line(points={{119,
          -190},{176,-190},{176,-192},{190,-192}}, color={0,0,127}));
