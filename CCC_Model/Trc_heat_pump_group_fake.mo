...
model Trc_heat_pump_group_fake
  ...
  CCC.Fluid.HeatPumps.Aixlib_AirToWaterHeatPump_3D_fake aixlib_HP_3D_integrated[
    numHeaPum](
  ...
  Modelica.Blocks.Routing.Replicator replicator3(nout=numHeaPum) annotation (
      Placement(transformation(
        ...
        rotation=270,
        origin={76,76})));
  Buildings.Fluid.Movers.SpeedControlled_y     mov[numHeaPum](
    ...
    annotation (Placement(transformation(extent={{110,8},{130,28}})));
  Buildings.Controls.OBC.CDL.Conversions.BooleanToReal booToRea[numHeaPum](realTrue
      =1)            "whether it is running domestic hot water or not"
...
equation
  ...
  connect(booToRea.y, mov.y) annotation (Line(points={{158,64},{166,64},{166,30},
