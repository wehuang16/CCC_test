...
parameter Integer numChi=2
  ...
  campus_chiller_plant.Plants.Subsequences.TesPlant tesPlant(m_flow_nominal=
        mCHWSec_flow_nominal)
    annotation (Placement(transformation(extent={{40,-24},{62,6}})));
  replaceable
    campus_chiller_plant.Plants.Subsequences.ElectricChillerParallelOriginal  pla(
    perChi=perChi,
    dTApp=dTApp,
    perCHWPum=perCHWPum,
    perCWPum=perCWPum,
  ...
  Buildings.Fluid.FixedResistances.Junction junDiv(
    ...
    dp_nominal={0,0,0}) annotation (Placement(transformation(
        ...
        rotation=180,
        origin={50,-54})));
  CCC_test.Bakersfield_Model.chiller_tes_plant_controller_original
