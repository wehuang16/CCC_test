...
model TestHeatPump
  ...
  Modelica.Blocks.Sources.Constant const3(k=273.15 + 50)
...
equation
  connect(const.y, hP_TES_water_test.T_closet) annotation (Line(points={{-47,80},
          {-16,80},{-16,53.3692},{-7.57143,53.3692}}, color={0,0,127}));
  connect(const1.y, hP_TES_water_test.Tdcw) annotation (Line(points={{-55,46},{
          -31.6,46},{-31.6,48.6615},{-7.57143,48.6615}}, color={0,0,127}));
  connect(const2.y, hP_TES_water_test.mdot_dcw) annotation (Line(points={{-45,-2},
          {-32,-2},{-32,43.1231},{-7.57143,43.1231}},     color={0,0,127}));
  connect(const3.y, hP_TES_water_test.TSupSet) annotation (Line(points={{73,-8},
          {76,-8},{76,33.4308},{35.5714,33.4308}}, color={0,0,127}));
  connect(const4.y, hP_TES_water_test.T_OA) annotation (Line(points={{73,-40},{
          86,-40},{86,37.3077},{35.5714,37.3077}}, color={0,0,127}));
  connect(booleanConstant1.y, hP_TES_water_test.HP_mode) annotation (Line(
        points={{51,82},{58,82},{58,41.4615},{35.5714,41.4615}}, color={255,0,
          255}));
  connect(booleanConstant.y, hP_TES_water_test.HP_On) annotation (Line(points={{19,-20},
          {46,-20},{46,29.2769},{35.5714,29.2769}},          color={255,0,255}));
  connect(const5.y, hP_TES_water_test.ResistanceThermalCapacity) annotation (
      Line(points={{-55,-38},{-44,-38},{-44,-36},{-20,-36},{-20,31.7692},{
          -7.57143,31.7692}}, color={0,0,127}));
  connect(booleanConstant2.y, hP_TES_water_test.Resistance_On) annotation (Line(
