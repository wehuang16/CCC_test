...
model ArisRoomModel_contained
  ...
  Buildings.Fluid.FixedResistances.PressureDrop ducCor1(
    ...
    m_flow_nominal=0.3*cor.V*1.2/3600)
    "Duct resistance (to decouple room and outside pressure)"
    annotation (Placement(transformation(extent={{438,-18},{418,2}})));
  ...
  Modelica.Blocks.Interfaces.RealOutput ZoneTAir1
    annotation (Placement(transformation(extent={{520,128},{540,148}})));
  Modelica.Blocks.Interfaces.RealOutput ZoneTAir2
    annotation (Placement(transformation(extent={{520,84},{540,104}})));
  Modelica.Blocks.Interfaces.RealOutput ZoneTAir3
    annotation (Placement(transformation(extent={{520,38},{540,58}})));
  Modelica.Blocks.Interfaces.RealOutput ZoneTAir4
    annotation (Placement(transformation(extent={{522,-26},{542,-6}})));
equation
  ...
  connect(qIntGai.y, cor1.qGai_flow) annotation (Line(points={{257,182},{262,
          ...
          {0,0,127}));
  connect(ducCor1.port_b, pAtm.ports[3]) annotation (Line(points={{418,-8},{292,
          -8},{292,-26},{196,-26},{196,62.5},{190,62.5}},      color={0,127,255}));
  ...
  connect(weaBus.TDryBul, TOut) annotation (Line(
      points={{154,152},{154,198},{514,198},{514,170},{530,170}},
      color={255,204,51},
      thickness=0.5), Text(
      string="%first",
      index=-1,
  ...
  connect(nor.TAir, ZoneTAir1) annotation (Line(points={{281,144},{384,144},{
          384,164},{514,164},{514,138},{530,138}}, color={0,0,127}));
  connect(cor.TAir, ZoneTAir2) annotation (Line(points={{343,52},{450,52},{450,
          94},{530,94}}, color={0,0,127}));
  connect(nor1.TAir, ZoneTAir3) annotation (Line(points={{441,156},{464,156},{
          464,142},{530,142},{530,48}}, color={0,0,127}));
  connect(cor1.TAir, ZoneTAir4) annotation (Line(points={{503,64},{514,64},{514,
          -16},{532,-16}}, color={0,0,127}));
  annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{100,
            -100},{520,200}})),                                  Diagram(
        coordinateSystem(preserveAspectRatio=false, extent={{100,-100},{520,200}})));
end ArisRoomModel_contained;
