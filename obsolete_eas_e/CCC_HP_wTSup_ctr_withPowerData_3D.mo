...
model CCC_HP_wTSup_ctr_withPowerData_3D
 ...
 parameter Modelica.Units.SI.MassFlowRate mCon_flow_nominal=0.575;
  ...
  Modelica.Blocks.Interfaces.RealOutput PEle(final unit="W", final displayUnit=
    ...
    annotation (Placement(
        ...
        rotation=90,
        origin={32.5,111.5})));
  Modelica.Blocks.Logical.Switch swiEvaFan "Switch fan at the evaporator side"
    annotation (Placement(transformation(
        extent={{-10,10},{10,-10}},
        origin={-80,40},
        rotation=0)));
  Modelica.Blocks.Sources.Constant const5(final k=0.1*mEva_flow_nominal)
    annotation (Placement(transformation(extent={{-126,20},{-116,30}})));
  Modelica.Blocks.Logical.Switch swiHeaCooYSet1
    "Switch ySet for heating and cooling" annotation (Placement(transformation(
        extent={{-10,10},{10,-10}},
        origin={118,-52},
        rotation=0)));
...
equation
  connect(sou.ports[1], mov.port_a) annotation (Line(points={{-46,8},{-28,8},{-28,
          20}},       color={0,127,255}));
  connect(mov.port_b,reversibleAirToWaterEuropeanNorm3D_withPowerData. port_a2)
    annotation (Line(points={{-28,40},{-28,42},{-14,42}}, color={0,127,255}));
  connect(reversibleAirToWaterEuropeanNorm3D_withPowerData.port_b2, sin.ports[1])
    annotation (Line(points={{6,42},{18,42},{18,6},{24,6}}, color={0,127,255}));
  connect(reversibleAirToWaterEuropeanNorm3D_withPowerData.port_b1, TWatSup.port_a)
  ...
  connect(reversibleAirToWaterEuropeanNorm3D_withPowerData.port_a1, TWatRet.port_b)
  ...
  connect(TOutAir,sou. T_in) annotation (Line(points={{113,-33},{-80,-33},{-80,12},
          {-68,12}},  color={0,0,127}));
  connect(reversibleAirToWaterEuropeanNorm3D_withPowerData.hea, Mode)
  ...
  connect(reversibleAirToWaterEuropeanNorm3D_withPowerData.QCon_flow, QCon_flow)
    annotation (Line(points={{-15,51.2},{-24,51.2},{-24,94},{-38,94},{-38,110}},
        color={0,0,127}));
  connect(reversibleAirToWaterEuropeanNorm3D_withPowerData.QEva_flow, QEva_flow)
    annotation (Line(points={{-15,48.6},{-30,48.6},{-30,68},{-4,68},{-4,110}},
        color={0,0,127}));
  connect(reversibleAirToWaterEuropeanNorm3D_withPowerData.PEle, PEle)
  ...
  connect(IO, swiEvaFan.u2) annotation (Line(points={{-112,-4},{-112,40},{-92,
          40}}, color={255,0,255}));
  connect(const.y, swiEvaFan.u3) annotation (Line(points={{-111,54},{-100,54},{
          -100,48},{-92,48}}, color={0,0,127}));
  connect(const5.y, swiEvaFan.u1) annotation (Line(points={{-115.5,25},{-102,25},
          {-102,32},{-92,32}}, color={0,0,127}));
  connect(const.y, mov.m_flow_in) annotation (Line(points={{-111,54},{-86,54},{
          -86,56},{-56,56},{-56,30},{-40,30}}, color={0,0,127}));
  connect(swiHeaCooYSet.y, hysteresis.u)
    annotation (Line(points={{33,-48},{58,-48}}, color={0,0,127}));
  connect(hysteresis.y, swiHeaCooYSet1.u2) annotation (Line(points={{81,-48},{
          96,-48},{96,-52},{106,-52}}, color={255,0,255}));
  connect(constTSetRooHea.y, swiHeaCooYSet1.u3) annotation (Line(points={{-61.5,
          -39},{-61.5,-40},{-18,-40},{-18,-32},{90,-32},{90,-44},{106,-44}},
        color={0,0,127}));
  connect(swiHeaCooYSet.y, swiHeaCooYSet1.u1) annotation (Line(points={{33,-48},
          {36,-48},{36,-60},{106,-60}}, color={0,0,127}));
  connect(swiHeaCooYSet.y, reversibleAirToWaterEuropeanNorm3D_withPowerData.ySet)
    annotation (Line(points={{33,-48},{33,50},{7.6,50},{7.6,50}}, color={0,0,
          127}));
  annotation (Diagram(coordinateSystem(extent={{-100,-60},{100,100}})), Icon(
        ...
        Rectangle(
          ...
          origin={0,-38},
          rotation=90)}));
end CCC_HP_wTSup_ctr_withPowerData_3D;
